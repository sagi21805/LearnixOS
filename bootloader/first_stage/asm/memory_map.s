...
2: # Int_15_setup
    xor ebx, ebx               # Clear continuation because it is the first call
    mov es, bx                 # Clear segment registers
    mov di, {map_address}      # Buffer address
    mov edx, {smap}            # 'SMAP' signature

3: # Int15 loop
    mov ecx, {extended_region_size}     # Buffer size
    mov eax, {function_code}   # Int15 function code
    clc                        # Clear carry flag just before the interrupt
    int 0x15
