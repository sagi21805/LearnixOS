...
use crate::{constants::GLOBAL_DESCRIPTOR_TABLE, first_stage, second_stage};


#[repr(C)]
struct GlobalDescriptorTableRegister32 {
    size: u16,
    offset: &'static GlobalDescriptorTable
}
...
impl AccessByte {
    ...
    pub const fn dpl(mut self, level: u8) -> Self { self.0 |= (level & 0x0f) << 5; self }
