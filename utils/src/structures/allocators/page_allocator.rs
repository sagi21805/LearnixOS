...
use core::{alloc::{GlobalAlloc, Layout}, borrow::BorrowMut, cell::UnsafeCell, panic, ptr::{null, Alignment}};

// Requires minimal of 32kib of memory in jumps of 32kib
use constants::{enums::PageSize, values::{BIG_PAGE_ALIGNMENT, BIG_PAGE_SIZE, HUGE_PAGE_ALIGNMENT, HUGE_PAGE_SIZE, REGULAR_PAGE_ALIGNMENT, REGULAR_PAGE_SIZE}};

use crate::{println, structures::paging::address_types};
...
impl BitMap {
    ...
    pub fn new(map_address: u64, memory_size: usize) -> BitMap {
        if memory_size.is_power_of_two() {
            unsafe { core::ptr::write_bytes(map_address as *mut u64, 0, (memory_size / REGULAR_PAGE_SIZE) / 64) };
            BitMap {
...
unsafe impl GlobalAlloc for PageAllocator {
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if layout.size() < REGULAR_PAGE_SIZE || layout.align() < REGULAR_PAGE_ALIGNMENT.as_usize() {
            ...
            let (map_index, bit_index) = self.find_free_region(
                PageSizeAlignment::from_layout(layout)
            );
            if bit_index >= 0 {
